...
最后更新：2026-09-09（第三阶段：可切换 Prisma 持久化层）
...
- 第三阶段将作为独立提交推送到同一分支；工作区发现 `workspace-preview.png` 的既有删除状态，本阶段不会恢复、删除或提交它。
...
1. 获得 PostgreSQL 环境后，执行 `db:push` / `db:seed`，并对 Prisma 分支做认证、解题并发、提示、会话与排行榜集成测试；再生成正式 migration。
2. 将控制器和前端 API 返回模型转换为 `packages/shared` 内的 DTO，收紧现有宽泛 `any`。
